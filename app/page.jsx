...
function limitDialogue(text, language) {
  ...
  const limit = dialogueLimits[language] || dialogueLimits.英文;
...
export default function Page() {
  ...
  const voiceScriptLimit = dialogueLimits[language] || dialogueLimits.英文;
  ...
  async function generateVideoClick() {
    ...
    try {
      ...
      setNotice('视频生成已提交。英文会走 Veo 3.1 Fast，自带英文口播。先生成关键图，再生成视频，请等待页面自动刷新。');
        ...
        <div className="flow-stack">
          <section className="flow-section">
            <div className="flow-action">
              ...
              <h3>先输入一个产品链接</h3>
              <p>提交后，后台只生成产品简报。面试演示版固定输出英文视频。</p>
                  ...
                  <div className="hook-list">
                    {filteredHooks.map((hook) => (
                      <button className={`hook-card ${hook.id === selectedHookId ? 'selected' : ''}`} key={hook.id} onClick={() => setSelectedHookId(hook.id)} type="button">
                        ...
                        <p className="hook-copy">{hook.scriptEn || hook.scriptZh}</p>
            ...
            <section className="flow-section revealed">
              ...
              {selectedHook ? (
                <div className="script-box">
                  {selectedScript ? (
                    <>
                      <div className="script-content">{selectedScript.scriptEn || selectedScript.scriptZh}</div>
                      <div className="tags">
